...
public function index() {
    ...
    $data['button_confirm'] = $this->language->get('button_confirm');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/paytabs')) {
  ...
  public function callback() {
	...
	$this->load->model('checkout/order');
 $secret_key = $this->config->get('payment_paytabs_password');
    ...
    if (isset($secret_key)) {
      ...
      $secret_key = $this->config->get('payment_paytabs_password');
      $paytabs_merchant = $this->config->get('payment_paytabs_merchant');
    ...
    if ($response == 100) {
      ...
      $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_paytabs_order_status_id'),false);
  ...
  if($order_info['payment_iso_code_3'] != 'USA' or $order_info['payment_iso_code_3'] != 'CAN') {
	 ...
	 $payment_data = array(		
			...
			'merchant_email'       => $this->config->get('payment_paytabs_merchant'),
			'secret_key'           => $this->config->get('payment_paytabs_password'),
      ...
      'cms_with_version'     =>' Open Cart 3.0.2',
      ...
      'return_url'           => $this->url->link('extension/payment/paytabs/callback', 'order_id=' .$this->session->data['order_id'], true)
